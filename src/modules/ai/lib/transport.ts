...
async function readNexisMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/NEXIS.md`;
  ...
  try {
    ...
    const content =
      r.content.length > NEXIS_MD_MAX_BYTES
        ? r.content.slice(0, NEXIS_MD_MAX_BYTES)
        : r.content;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readNexisMd(live.workspaceRoot);
