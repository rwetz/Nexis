...
const INIT_PROMPT = `Scan this workspace and produce NEXIS.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap NEXIS.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const NEXIS_CMD_RE =
  /^<nexis-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<nexis-command name="${name}" />\n\n${prompt}`;
}
