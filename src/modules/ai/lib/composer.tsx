...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nexis:ai-attach-file", onAttach);
    return () => window.removeEventListener("nexis:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the NEXIS.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<nexis-command name="${outcome.commandName}" />`;
        }
      }
    }
