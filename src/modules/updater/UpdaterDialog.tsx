...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S nexis-bin";
    case "debian":
      return `sudo apt install ./Nexis_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Nexis-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Nexis v${manual.version} is available`
                  : `Nexis v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Nexis to finish installing."
              : downloading
                ? progress !== null
