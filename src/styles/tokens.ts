...
// ---------------------------------------------------------------------------
// Ad-hoc colour resolution
// ---------------------------------------------------------------------------

/**
 * Resolve any CSS colour expression (including `oklch(...)` and `var(--x)`)
 * to the `rgb(r, g, b)` string canvas 2D accepts everywhere. Same probe
 * trick as readAppTokens, exposed for consumers that mint colours at runtime
 * rather than reading a fixed token list — the isometric renderer builds its
 * language ramp in OKLCH and needs concrete channels to shade box faces.
 */
export function resolveCssColor(value: string): string {
  return toRgbString(resolve0(value));
}

/**
 * Chromium keeps `oklch()` in computed styles rather than serialising it to
 * `rgb()`, so the probe alone is no longer enough for canvas consumers that
 * want channels. Painting the value into a 1x1 canvas gets us real numbers
 * for any colour the browser can parse. Alpha is preserved: dark-mode
 * hairlines are `oklch(1 0 0 / 10%)`, and flattening that to opaque white
 * would be badly wrong.
 */
function toRgbString(value: string): string {
  if (value.startsWith("rgb")) return value;
  const ctx = getConverter();
  if (!ctx) return value;
  ctx.clearRect(0, 0, 1, 1);
  ctx.fillStyle = "#000";
  ctx.fillStyle = value; // invalid values leave the previous style in place
...
let converter: CanvasRenderingContext2D | null = null;

function getConverter(): CanvasRenderingContext2D | null {
  if (converter) return converter;
  const canvas = document.createElement("canvas");
  canvas.width = 1;
  canvas.height = 1;
  converter = canvas.getContext("2d", { willReadFrequently: true });
  return converter;
}

function resolve0(value: string): string {
  if (!probe) {
    probe = document.createElement("div");
    probe.style.position = "absolute";
    probe.style.visibility = "hidden";
