...
- **Atlas**: every git repo on your machine in one place — a status list (branch, ahead/behind, dirty counts, stashes, last commit) and an isometric map where repos are plots, files are buildings, and height is lines of code. One scan, two views, one shared selection. Open any repo as a workspace or a terminal tab from either.
- **Benchmark**: measure local models across inference backends. Drop in `.onnx` or `.gguf` files, run the model x backend matrix, and compare throughput, latency, peak memory and accuracy side by side — real inference through ONNX Runtime and llama.cpp, real training throughput through nexis-ml, and a simulated backend for everything else. Every result says which it was.
