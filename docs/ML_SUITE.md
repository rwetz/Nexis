...
- The roadmap's hard limits: "every dependency earns its place," and no bundled inference engine. The binary budget that originally framed this argument is gone (loosened to a regression tripwire in 2026-09), and **the conclusion moved with it in v1.27.0**: absorbing the Benchmark app brought ONNX Runtime in as a linked dependency, because a benchmarking tool whose only real backend is a simulation is not a benchmarking tool. See `docs/vault/decisions/bundling-onnx-runtime.md`.

  What has *not* changed is the argument as it applies to **training**, which is what this document is about. libtorch is ≈ 2 GB and never fit under any ceiling; a training engine brings a per-backend build matrix and a GPU-driver support surface that an external engine keeps out of this repo entirely. ONNX Runtime is a *fixed-size inference* dependency with none of that shape, which is exactly why it was the one that could come in. Training still lives in `nexis-ml`, spawned as a separate process.
...
**`weights` artifact contract (Tier-2 network graph — both sides shipped: Nexis renders it, `nexis-ml-rs` v0.8 emits it per eval for tabular runs ≤200k weights).** The panel's NetworkGraph renders the architecture from train.toml today; when an engine emits a per-eval `{"ev":"artifact","kind":"weights","path":".nexis-ml/runs/<id>/artifacts/weights-epochN.json"}`, the graph overlays learned connection strengths and re-renders every eval (watch connections strengthen/die during training). File format the engine must write:

```jsonc
{
  "layers": [
    // dense layers only, forward order; w[i][j] = weight from in-unit i to out-unit j
    { "in": 8, "out": 64, "w": [[0.12, -0.4 /* …8×64 floats */]] }
  ]
}
```

Nexis matches layers to the drawn columns by `in`/`out` sizes and bucket-averages |w| for display (`src/modules/ml/lib/netgraph.ts`), so the engine may also downsample large layers as long as `in`/`out` reflect the emitted matrix. Layers whose sizes don't match the toml-derived architecture are skipped gracefully. Parser is defensive (`parseWeights`) — a malformed file renders structure-only, never throws.
