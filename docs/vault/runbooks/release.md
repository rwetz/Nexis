...
`.github/workflows/release.yml` runs on `push: tags: v*` — it builds the macOS, Linux and Windows artifacts, enforces the **<10 MB binary-size budget**, and creates the GitHub release. Concurrency is one release build at a time, and it does **not** cancel in progress, so a mistaken tag can't be fixed by racing a second one — delete the tag remote-side and re-push.
