...
- **`sysinfo` is pinned feature-minimal** (`system`, `disk`, `network`). Defaults pull `component`, `user`, and rayon. The dep costs ~168 KiB — re-measure before adding a feature. The release budget it was weighed against is now a loose 40 MB tripwire, so this is about keeping the dependency honest rather than about clearing a gate.
