...
Web Tools, Atlas, Benchmark, Source Control, and Explorer are declarative panels. Atlas and Benchmark also declare their focused companion windows, so the titlebar and window shell consume capability metadata instead of maintaining parallel tool lists. Source Control and Explorer keep their existing shell-owned rail rows for a live badge and core navigation while capability code owns rendering and command activation. Benchmark's module-scoped event listeners keep outliving panel mounts.
