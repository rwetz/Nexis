...
Web Tools and embedded Atlas are the first declarative panels. Atlas's companion window and remaining panels still use legacy shell composition pending capability migration. Its scan/store remain domain-owned. Benchmark event listeners must keep outliving panel mounts when migrated.
