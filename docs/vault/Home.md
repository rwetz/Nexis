...
- [[platform]] — typed IPC, workspace scope, storage, process, filesystem, window and notification policy
...
- [[debugger]] — DAP adapter sessions, typed host protocol, event routing and store lifetime
...
- `decisions/` — lightweight ADRs: why something is the way it is, alternatives rejected. [[expansion-packs]], [[nexis-ml-artifact-pinning]], [[program-resolution-before-spawn]], [[command-ledger]], [[absorbing-the-nexis-apps]], [[bundling-onnx-runtime]], [[navigation-surfaces]] (new ones from `templates/decision.md`)
