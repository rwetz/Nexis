...
- This is host-side resolution only. WSL tools resolve inside the distro via `command -v` through `wsl_exec_capture` — see pitfall #20 in AGENTS.md for why the two sides must not answer for each other.
