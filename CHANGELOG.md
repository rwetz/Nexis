...
- **`ZED_INSPIRATION.md`** — research notes on Zed applied to Nexis, two halves: (1) zed.dev/docs information architecture worth stealing for nexis-wiki ("Coming From…" migration guides, generated all-settings reference, first-class Privacy & Security section, per-platform troubleshooting, "Developing Nexis" in the user docs — mirrored into ROADMAP's new wiki backlog); (2) Rust performance lessons that survive the translation from a native GPU editor to a Tauri app (benchmark-first culture → the Criterion harness above, non-shipping cargo profiles, workspace clippy lints, SumTree/snapshot patterns as the reference design for the ROADMAP TS→Rust migration item). Collab/multiplayer explicitly out of scope.
- **`TERAX_INSPIRATION.md`** — survey of upstream terax-ai v0.6.4→v0.8.5 (features worth adopting: block-mode terminal *without* a renderer rewrite via OSC 133 + input gating, Spaces/tab groups, OSC 52 clipboard, MRU tab switcher, offline whisper.cpp voice, close-confirm on running processes) plus Linux/Windows optimization research: their 0.8.0 renderer-lifecycle perf pass (WebGL/slot reaping, TUI keep-alive, motion→CSS) against Nexis's platform-first work. Actionable perf items mirrored into ROADMAP's hardening backlog; the landed "lazy-load debugger/database panels" backlog item (shipped in 1.20.6) was removed while updating it.
