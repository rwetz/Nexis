...
- **The Python engine install said only which interpreter it would use, not what that meant.** With no virtualenv anywhere in the workspace or its ancestors, the install target falls back to the machine-wide interpreter — so "Installs into `C:\Python314\python.exe`" was silently proposing to add ~3 GB of PyTorch to the Python every project on the machine shares, and nothing said so. The line now names the *kind* of environment (project virtualenv / conda / machine-wide) and its version, and a system target explains what that implies and points at the two contained alternatives.
  - **A too-new interpreter is now called out before the download, not after.** PyTorch publishes no wheels for a CPython until weeks-to-months after its release, and pip only discovers this once it has resolved — so an install on Python 3.14 spends minutes downloading and ends at "No matching distribution found for torch", which reads like a network fault. `lib/pythonSupport.ts` brackets the range and warns up front, naming the standalone engine as the way through. It is deliberately a warning and never a block: the bound is a heuristic that goes stale in one direction only, and a stale bound must not stop someone whose environment actually works.
