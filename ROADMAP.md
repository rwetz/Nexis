...
- [ ] **Expansion packs — core + opt-in feature surface** — the sidebar's ~24 panels split into a fixed core (terminal, editor, Files, Recent Files, Source Control, AI chat via API) plus toggleable packs (navigation-plus, code-tools, ai-extras, dev-tools, ml-lab, advanced). Enablement gating, not installation — nothing is downloaded except the future nexis-ml flow. Taxonomy and decisions in `docs/vault/decisions/expansion-packs.md`; taxonomy source of truth in `src/lib/packs.ts`.
  ...
  - [ ] **V2 — finish the gating mechanism**: gate command-palette entries + keybindings targeting disabled packs (the visible V1 leak — do this first); migrate hardwired panels into the plugin registry one per PR as they're touched (Debugger/Database/Advanced group first — already lazy-loaded); hide pack-owned settings (e.g. ML toggles) when the pack is off
  - [ ] **V3 — nexis-ml install flow** (the only real download): consent prompt, pinned checksum/signature on the artifact, Rust vs Python variant selection, offline "use a local copy" path, uninstall + disk-usage readout. **Blocked on a design decision: artifact hosting + signing story**
  - [ ] **V4 — polish, driven by real usage**: "this tab needs the X pack — enable it?" placeholder on session restore instead of silent fallback; discoverability for Bare-Bones users; per-pack settings / simpler feature variants only if users ask (deliberately cut from V1)
