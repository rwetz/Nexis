...
Nexis direction, what's shipped, what's coming, and what's deliberately out of scope.

This file is updated as direction evolves. For day-to-day work, see [GitHub Issues](https://github.com/rwetz/Nexis/issues) and the Projects board.

## What Nexis is

Nexis is a fast, lightweight, AI-native terminal (ADE - agentic development environment). It pairs a native PTY backend with a modern UI: multi-tab terminals, an integrated code editor, a file explorer, source control, and a first-class AI agent system that works with your own API keys or fully local models. Under 10 MB on disk. No telemetry. Keys stored in the OS keychain.
...
- [ ] AI agent meta-orchestration (Nexis agent spawning and managing external coding agents like Claude Code / OpenCode)
...
Categories that will not be built into Nexis. Individual feature requests in these categories will be closed.
...
- **Notebook and document workspaces.** Anything that turns Nexis into a document host rather than a terminal.
...
- **Telemetry, analytics, accounts.** Nexis stays BYOK and offline-respectful.
...
- **Third-party subscription session bridges.** Forwarding cloud subscription auth (provider-managed login sessions) through Nexis is not technically feasible for third-party clients.

## Decision authority

Direction and scope decisions are made by [@rwetz](https://github.com/rwetz). Trusted reviewers (informal, no fixed roles yet) provide input on security, performance, and platform-specific areas.

If a PR is closed and you disagree, open a GitHub Discussion or comment on the relevant issue. Happy to discuss, not happy to be ambushed in a PR comment thread.
