...
carry. The binary ships at **29.7 MiB** as a result, up from ~9.5 MiB. The reasoning is in
`docs/vault/decisions/bundling-onnx-runtime.md`, and it is the decision to argue with — not the
number.
...
- [ ] **Web Dev pack** — **the three core panels all ship**: the multi-viewport preview (phone/tablet/
  desktop side by side against one dev server, CSS-pixel widths, one shared scale), the HTTP/REST client
  (per-workspace saved requests and variables, through a new SSRF-guarded `http_send` kept separate from
  the AI's egress), and the scratchpad tools (JSON formatter with a deliberate JSONPath subset, a JWT
  decoder that states it decodes rather than verifies, a regex tester, base64/URL codecs). Lower priority and worth re-deciding before
...
- [ ] **Atlas: scan inside a WSL distro** — Atlas is a host-machine inventory and deliberately stays host-scoped when the workspace is WSL (the panel's status line says so, and the reasoning is in `src-tauri/src/modules/atlas/mod.rs` and `docs/vault/decisions/absorbing-the-nexis-apps.md`). Scanning a distro is a real feature and not a small one: `libgit2` cannot read `\\wsl.localhost\...` at the speed the panel assumes, so this most likely means running the walk *inside* the distro and returning a summary — which is a second scanner, not a parameter. Two rules from pitfall #20 apply if it is built: a path handed to `wsl.exe` must be the caller's Linux path, and every cached answer must key on the workspace scope. Until then the panel's job is to be honest about what it read, which it is.
- [ ] **Benchmark: a GPU execution provider** — ONNX Runtime is linked CPU-only (`docs/vault/decisions/bundling-onnx-runtime.md`). A GPU path is the execution-provider feature, the dylibs it drags in, and per-platform `bundle.resources` config to actually ship them — i.e. exactly the build-matrix and driver-support surface the original no-bundled-engine argument objected to. Worth doing only if someone actually wants to benchmark GPU inference, and worth scoping as its own change if so.
