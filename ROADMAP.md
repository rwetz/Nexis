...
- [x] ~~**Quick terminal — global-hotkey drop-down overlay.**~~ — done (see CHANGELOG `[Unreleased]`). Built as described: `tauri-plugin-global-shortcut` plus a hidden-not-destroyed borderless window on `/?quick=1` reusing zen mode and the existing PTY stack. *Deliberately cut from V1:* an in-app hotkey recorder (the accelerator is a pref with a validated format, but there is no press-a-key-to-rebind UI yet) and per-monitor position memory.
- [x] ~~**"Explain / Fix" on a failed command**~~ — done (see CHANGELOG `[Unreleased]`).

**AI / agent**
- [x] ~~**Natural-language → command (Warp's "AI Command Search").**~~ — done: Ctrl/Cmd+Shift+I bar over the terminal; the suggestion is inserted at the prompt, never executed, so the user's Enter is the confirmation (see CHANGELOG `[Unreleased]`).
- [x] ~~**Parameterized command workflows.**~~ — already in the product before the survey: the Shell Snippets panel (Advanced pack) supports `{VAR}` placeholders with a fill-form-before-run (`extractPlaceholders`/`fillPlaceholders` in `ShellSnippetsPanel.tsx`; the syntax is documented in the panel footer). Remaining delta vs Warp Drive — per-arg defaults and descriptions — is deliberately untracked until someone asks.
- [x] ~~**Scoped auto-approve mode for trusted, low-risk commands.**~~ — done: the `auto-safe` policy on `bash_run` (see CHANGELOG `[Unreleased]`). User-editable approval patterns were deliberately cut — the curated allowlist *is* the pattern set; revisit only if real usage demands it.
...
- [x] ~~AI command audit log~~ — done (see CHANGELOG `[Unreleased]`). The pattern-approval half was deliberately deferred to ship together with an auto-approve mode; that mode has since shipped too (the `auto-safe` policy — see the AI/agent backlog above), and the log now records how each command's gate was passed.
...
- [x] ~~Verify frontend-generated device-query replies (DA/DSR/CPR) route through the ordered per-session PTY writer path~~ — done: verified immune (replies ride the same `onData` → `pty_write` → writer-channel path as keystrokes) and the two load-bearing properties are now tripwired (see CHANGELOG `[Unreleased]`).
