...
pub async fn git_stash_push(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    blocking(app, move |r| {
        operations::stash_push(r, &repo_root, message.as_deref(), &workspace).map_err(Into::into)
    })
    .await
}
...
pub async fn git_stash_apply(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    blocking(app, move |r| {
        operations::stash_apply(r, &repo_root, &stash_ref, &workspace).map_err(Into::into)
    })
    .await
}
...
pub async fn git_stash_pop(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    blocking(app, move |r| {
        operations::stash_pop(r, &repo_root, &stash_ref, &workspace).map_err(Into::into)
    })
    .await
}
