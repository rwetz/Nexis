...
fn generate_sentinel() -> String {
    ...
    format!("__NEXIS_CWD_{:016x}_{:016x}__", mix, counter)
}
...
impl ShellSession {
    ...
    pub fn current_cwd(&self) -> String {
        // Poison recovery on the cwd mutex (pitfall #8 pattern) — a panicked
        // holder must not brick every later command in this session.
...
fn wrap_with_sentinel(command: &str, workspace: &WorkspaceEnv, sentinel: &str) -> String {
    ...
    {
        format!(
        "{command}\n$__nexis_rc = if ($null -ne $LASTEXITCODE) {{ $LASTEXITCODE }} elseif ($?) {{ 0 }} else {{ 1 }}\n\"`n{sentinel}$($PWD.Path)\"\nexit $__nexis_rc\n",
    )
    }
}
...
mod tests {
    ...
    fn sentinels_are_unique_per_session() {
        ...
        assert!(a.sentinel.starts_with("__NEXIS_CWD_"));
    ...
    fn strip_uses_session_sentinel_only() {
        ...
        let attacker = "__NEXIS_CWD_0000000000000000_0000000000000000__/evil";
        let trailer = format!("\n{}/real\n", s.sentinel);
    ...
    fn wrap_posix_includes_pwd_print_and_exit_code() {
        ...
        assert!(
            ...
            "must use printf to emit sentinel"
        );
    ...
    fn wrap_windows_uses_psd_path_not_pwd() {
        ...
        assert!(
            wrapped.contains("$PWD.Path"),
            "Windows wrap must use $PWD.Path"
        );
        assert!(wrapped.contains(&s.sentinel));
    }
    ...
    fn strip_extracts_cwd_and_trims_trailing_newlines_from_clean_output() {
        ...
        assert_eq!(
            clean, "line1\nline2",
            "trailing newlines before sentinel must be stripped"
        );
