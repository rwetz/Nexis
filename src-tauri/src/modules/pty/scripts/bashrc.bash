...
# nexis-shell-integration (bashrc)
#
...
if [ -z "$__NEXIS_HOOKS_LOADED" ]; then
  ...
  _nexis_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_nexis_urlencode "$PWD")"
    if [ -z "$__NEXIS_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __NEXIS_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_nexis_precmd:"*) ;;
    *) PROMPT_COMMAND="_nexis_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
