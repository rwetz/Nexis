...
# zsh, so we shadow $? into `_nexis_ret`.

{
  _nexis_user_zdotdir="${NEXIS_USER_ZDOTDIR:-$HOME}"
  [ -f "$_nexis_user_zdotdir/.zshrc" ] && source "$_nexis_user_zdotdir/.zshrc"
...
if [[ -z "$__NEXIS_HOOKS_LOADED" ]]; then
  __NEXIS_HOOKS_LOADED=1
  ...
  _nexis_precmd() {
    local _nexis_ret=$?
    printf '\e]133;D;%s\e\\' "$_nexis_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_nexis_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _nexis_precmd
    add-zsh-hook preexec _nexis_preexec
  fi
